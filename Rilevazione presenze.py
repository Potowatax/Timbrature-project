...
class Timbrature:
    ...
    def calcola_saldo(self, entrate, uscite, pausa_pranzo, attivita, rientro, causali_uscita, causali_entrata):
        ...
        # Gestione delle causali "RECUPERO" e "STRAORDINARIO"
        if "RECUPERO" in causali_entrata + causali_uscita:
            if saldo < timedelta():
                saldo = timedelta()
            else:
                saldo = min(saldo, totale_lavorato - ore_richieste)

        if "STRAORDINARIO" in causali_entrata + causali_uscita:
            if totale_lavorato > ore_richieste:
                straordinario = totale_lavorato - ore_richieste
                if "PAUSA_PR" not in causali_entrata + causali_uscita:
                    straordinario -= timedelta(minutes=30)
                saldo = straordinario
